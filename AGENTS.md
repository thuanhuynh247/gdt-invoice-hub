...
This repo uses Harness v2.0 (Hermes-Inspired multi-agent orchestration). Before work, read:
...
5. **Run Validation wrapper**: Execute all testing and compile tasks via the validation wrapper: `scripts/harness validate --cmd "<test-command>"`. Never run raw tests outside of it.
6. **Telemetry & Trace**: Record your turn execution trace using `scripts/harness trace ...` immediately upon completion.
7. **Self-Improving Loop**: Update local runbooks (`SKILL.md` documents) or `AGENTS.md` if any operational friction or errors are resolved during execution.
<!-- HARNESS:END -->
