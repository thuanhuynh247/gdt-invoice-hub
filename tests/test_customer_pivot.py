"""Tests for monthly customer pivot table and excel exporter endpoints."""
...
def _seed_test_invoices(app):
    """Seed sales invoices and taxpayer profiles for customer pivot tests."""
    with app.app_context():
        ...
        # Invoice 1: Jan 2026, Seller is us (active taxpayer), Buyer is Customer A
        inv1 = Invoice(
            id="MST_A-C26-0001",
            filename="sales_invoice_1.xml",
            invoice_type="sales",
            template_code="1/001",
            symbol="C26",
            number="0001",
            date="2026-01-15",
            currency="VND",
            seller_mst="0109999999",  # active taxpayer MST
        ...
        # Invoice 2: Feb 2026, Seller is us (active taxpayer), Buyer is Customer A
        inv2 = Invoice(
            id="MST_A-C26-0002",
            filename="sales_invoice_2.xml",
            invoice_type="sales",
            template_code="1/001",
            symbol="C26",
            number="0002",
        ...
        # Invoice 3: Feb 2026, Seller is us (active taxpayer), Buyer is Customer B
        inv3 = Invoice(
            id="MST_B-C26-0003",
            filename="sales_invoice_3.xml",
            invoice_type="sales",
            template_code="1/001",
            symbol="C26",
            number="0003",
        ...
        # Invoice 4: Feb 2026, Seller A, purchase (should NOT be included in sales pivot)
        inv4 = Invoice(
            id="MST_A-C26-0004",
            filename="pur_invoice_4.xml",
            ...
            seller_name="KHÁCH HÀNG A",
            buyer_mst="0109999999",  # active taxpayer is buyer
        ...
        # Invoice 5: Feb 2026, Seller is other, Buyer is Customer A, OTHER taxpayer (tenant isolation)
        inv5 = Invoice(
            id="MST_A-C26-0005",
            filename="sales_invoice_other.xml",
            invoice_type="sales",
            template_code="1/001",
            symbol="C26",
            number="0005",
            date="2026-02-20",
            currency="VND",
            seller_mst="0333333333",  # Different seller MST
            seller_name="CONG TY KHAC",
            buyer_mst="0101234567",
            buyer_name="KHÁCH HÀNG A",
...
def test_customer_pivot_requires_login(client):
    """Verify anonymous users are rejected with 401."""
    response = client.get("/api/invoices/customer-pivot")
    assert response.status_code == 401

    response = client.get("/api/invoices/customer-pivot/export")
    assert response.status_code == 401


def test_customer_pivot_json_success(logged_in_client, app):
    """Verify JSON pivot engine filters and aggregates correctly under tenant isolation."""
    _seed_test_invoices(app)

    response = logged_in_client.get("/api/invoices/customer-pivot?year=2026&value_type=total_amount&taxpayer_mst=0109999999")
    ...
    rows = payload["rows"]
    # We should have 2 buyers in the sales list: KHÁCH HÀNG A and KHÁCH HÀNG B
    # Sorted by grand total descending, so KHÁCH HÀNG A (1.1M + 2.2M = 3.3M) is first, KHÁCH HÀNG B (550k) is second
    assert len(rows) == 2
    
    row_a = rows[0]
    assert row_a["buyer_mst"] == "0101234567"
    assert row_a["buyer_name"] == "KHÁCH HÀNG A"
    ...
    row_b = rows[1]
    assert row_b["buyer_mst"] == "0209876543"
    assert row_b["buyer_name"] == "KHÁCH HÀNG B"
...
def test_customer_pivot_json_invoice_count(logged_in_client, app):
    """Verify pivot counts aggregate correctly."""
    _seed_test_invoices(app)

    response = logged_in_client.get("/api/invoices/customer-pivot?year=2026&value_type=invoice_count&taxpayer_mst=0109999999")
    assert response.status_code == 200
    payload = response.get_json()
    assert payload["success"] is True

    rows = payload["rows"]
    # Buyer A: 1 in Jan, 1 in Feb = 2 total
    assert rows[0]["buyer_mst"] == "0101234567"
    assert rows[0]["monthly_values"]["01"] == 1
    assert rows[0]["monthly_values"]["02"] == 1
    assert rows[0]["row_total"] == 2

    # Buyer B: 0 in Jan, 1 in Feb = 1 total
    assert rows[1]["buyer_mst"] == "0209876543"
...
def test_customer_pivot_export_xlsx(logged_in_client, app):
    """Verify Excel download route returns a valid formatted spreadsheet."""
    _seed_test_invoices(app)

    response = logged_in_client.get("/api/invoices/customer-pivot/export?year=2026&value_type=total_amount&taxpayer_mst=0109999999")
    assert response.status_code == 200
    ...
    # Title is in row 1
    assert "BẢNG TỔNG HỢP HOÁ ĐƠN ĐẦU RA THEO KHÁCH HÀNG" in sheet["A1"].value
